...
def register_planner_scenarios(provider: object) -> None:
    ...
    The first registered scenario uses the prompt prefix ``"Generate up to"``
    which matches the real prompt generated by ``build_test_generation_prompt()``.
    Other scenarios use test-specific identifiers that are injected by test code.
    ...
    # Day 6 fix: changed "generate test" → "Generate up to" to match
    # the real prompt prefix from build_test_generation_prompt().
    # With the case-insensitive matching in MockLLMProvider (also Day 6),
    # this now reliably matches the real planner prompt.
...
def register_default_planner_scenario(provider: object) -> None:
    """Register only the valid test plan scenario for the real planner prompt.

    This is a convenience function for tests that only need the
    happy-path mock response to work with the real ``LLMTestPlanner``.
